...
const VerificationPage: React.FC = () => {
  ...
  const { verifyEmail, resendVerificationEmail } = useContext(AuthContext); // Add resendVerificationEmail from AuthContext
  ...
  const handleResendCode = async () => {
    setIsResending(true);
    try {
      await resendVerificationEmail(); // Call the resendVerificationEmail method
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-white">
      <div className="max-w-md w-full bg-gray-100 bg-opacity-50 backdrop-filter backdrop-blur-xl rounded-2xl shadow-xl overflow-hidden">
        ...
        >
          <h2 className="text-3xl font-bold mb-6 text-center bg-gradient-to-r from-olive to-forestGreen text-transparent bg-clip-text">
            Verify Your Email
          </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <motion.button
              ...
              className="w-full bg-gradient-to-r from-olive to-forestGreen text-white font-bold py-3 px-4 rounded-lg shadow-lg hover:from-green-600 hover:to-emerald-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50 disabled:opacity-50"
            >
